  ...
  function createRecruiterCoreApi() {
    ...
    function formatVndInput(el) {
      ...
      try {
        ...
        var digitsBefore = (raw.slice(0, start).match(/\d/g) || []).length;
        var numeric = raw.replace(/[^\d]/g, '');
        var formatted = numeric ? Number(numeric).toLocaleString('vi-VN') + 'đ' : '';
        el.value = formatted;

        // place cursor after the same count of digits
        if (digitsBefore > 0 && formatted) {
          var pos = 0, counted = 0;
